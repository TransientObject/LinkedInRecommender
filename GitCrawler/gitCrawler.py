...
def login_git():
	gh = login('SattyS', password='***', token='d2a4d66b7825ca378c5f')
	return gh
...
def getJSONresult(gh):
	#relatedQueryDict = defaultdict(list)
	main_query_list = getSkills()
	related_query_list = []
	fullQueryList = []
	gsrList = []
	for query in main_query_list:
		related_query_list.append(getRelatedQueryTerms(query,2))
	...
	for query in main_query_list:
		...
		for related_query in related_query_list[k]:
			gsrList.append(search_repo(related_query,5,gh,query))
		k += 1
